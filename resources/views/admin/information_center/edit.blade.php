...
<form action="{{ route('admin.information-center.update', $informationCenter->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 mb-6">
        ...
        <select name="category" id="category-select" required class="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-2xl text-sm font-bold focus:ring-2 focus:ring-usu-green/20 focus:border-usu-green transition-all cursor-pointer">
            ...
            <option value="maintenance" {{ old('category', $informationCenter->category) == 'maintenance' ? 'selected' : '' }}>Pemeliharaan Server / Gedung (Maintenance)</option>
            <option value="new_collection" {{ old('category') == 'new_collection' || $informationCenter->category == 'new_collection' ? 'selected' : '' }}>Buku / Koleksi Baru</option>
            <option value="tips" {{ old('category', $informationCenter->category) == 'tips' ? 'selected' : '' }}>Tips & Trik</option>
            <option value="promotion" {{ old('category', $informationCenter->category) == 'promotion' ? 'selected' : '' }}>Promo / Penawaran khusus</option>
            <option value="general" {{ old('category', $informationCenter->category) == 'general' ? 'selected' : '' }}>Informasi Umum</option>
        </select>
    </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8">
                ...
                <div class="space-y-5">
                    ...
                    <div id="trix-editor-container">
                        ...
                        <input id="content" type="hidden" name="content" value="{{ old('content', $isJson ? ($contentDecoded['description'] ?? '') : $informationCenter->content) }}">
                    ...
                    <div id="fields-maintenance" class="category-fields-section hidden space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="maintenance_services" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: Website OPAC, Aplikasi Mobile Perpustakaan" value="{{ old('maintenance_services', $isJson ? ($contentDecoded['affected_services'] ?? '') : '') }}">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-2">Perkiraan Durasi / Selesai</label>
                                <input type="text" name="maintenance_downtime" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: 2 Jam (12.00 - 14.00 WIB) atau Selesai Gedung Dibersihkan" value="{{ old('maintenance_downtime', $isJson ? ($contentDecoded['estimated_downtime'] ?? '') : '') }}">
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-2">Link / Solusi Alternatif (Pengalihan Jika Ada)</label>
                            <input type="url" name="maintenance_alternative" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: https://alternative-opac.usu.ac.id" value="{{ old('maintenance_alternative', $isJson ? ($contentDecoded['alternative_link'] ?? '') : '') }}">
                        </div>
                    </div>
                    ...
                    <div id="fields-new_collection" class="category-fields-section hidden space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="book_title" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: Algoritma & Pemrograman" value="{{ old('book_title', $isJson ? ($contentDecoded['book_title'] ?? '') : '') }}">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-2">Penulis / Pencipta <span class="text-red-500">*</span></label>
                                <input type="text" name="book_author" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: Prof. Dr. Budi Luhur" value="{{ old('book_author', $isJson ? ($contentDecoded['book_author'] ?? '') : '') }}">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-2">Penerbit & Tahun Terbit</label>
                                <input type="text" name="book_publisher" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: Erlangga, 2024" value="{{ old('book_publisher', $isJson ? ($contentDecoded['book_publisher'] ?? '') : '') }}">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-2">Lokasi Rak / Klasifikasi Buku</label>
                                <input type="text" name="shelf_location" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: Rak 4B - Umum atau Klasifikasi DDC 005.1" value="{{ old('shelf_location', $isJson ? ($contentDecoded['shelf_location'] ?? '') : '') }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="fields-promotion" class="category-fields-section hidden space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="promo_period" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: 1 - 31 Agustus 2026" value="{{ old('promo_period', $isJson ? ($contentDecoded['promo_period'] ?? '') : '') }}">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-2">Benefit / Hadiah Promo <span class="text-red-500">*</span></label>
                                <input type="text" name="promo_benefit" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: Bebas Denda Keterlambatan atau Akses Jurnal Gratis" value="{{ old('promo_benefit', $isJson ? ($contentDecoded['promo_benefit'] ?? '') : '') }}">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div id="card-narahubung" class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="contact_phone" value="{{ old('contact_phone', $informationCenter->contact_phone) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder="Contoh: 08123456789">
                    </div>
                    <div>
            ...
            <div id="card-pengaturan-tampilan" class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $informationCenter->sort_order) }}" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm">
        ...
        imageInput.addEventListener('change', function() {
            ...
            if (file) {
                ...
                @if($informationCenter->image_path)
                    imagePreview.src = "{{ asset($informationCenter->image_path) }}";
                @else
